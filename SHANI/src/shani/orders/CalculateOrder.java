...
public class CalculateOrder extends KeywordOrder {
	...
	private static double calculate(String expresion) throws Exception {
		...
		Object el2=elements.get(0);
		for(int i=0;i<elements.size()-1;i++) {				//add negative numbers and fraction
			Object el=el2;
			...
			if(el instanceof Operation&&((Operation) el).type=='-') {
				...
				if(el2 instanceof Operation&&(((Operation) el2).type==','||((Operation) el2).type=='.')) {
					...
					if(el3 instanceof Value) {
						...
						if(((Value) el).value>0)((Value) el).value+=val/divider;
						else ((Value) el).value-=val/divider;
						elements.remove(i+2);
						elements.remove(i+1);
					} else throw new Exception();				//illegal use of ',' or '.'
				}
			}
		}
		...
		for(int i=0;i<elements.size()-1;i++) {
			...
			el2=elements.get(i+1);
			if(el instanceof Operation&&((Operation) el).type=='^') {
				if(i==0||!(el1 instanceof Value)||!(el2 instanceof Value))throw new Exception();					//Illega use of '^'
		...
		if((el1=elements.get(elements.size()-1))instanceof Operation&&((Operation)el1).type=='^') throw new Exception();		//Illega use of '^'
